...
#define AXI_LCD_STATUS_VMDA_UNDERFLOW	BIT(4)
#define AXI_LCD_STATUS_VMDA_OVERFLOW	BIT(3)
#define AXI_LCD_STATUS_VMDA_BE_ERROR	BIT(2)
#define AXI_LCD_STATUS_VMDA_TPM_OOS	BIT(1)
#define AXI_LCD_STATUS_HDMI_TPM_OOS	BIT(0)

#define AXI_LCD_COLOR_PATTERN_ENABLE	BIT(24)

#define AXI_LCD_REG_RESET		0x040
#define AXI_LCD_REG_CTRL		0x044
...
#define AXI_LCD_REG_HTIMING2		0x404
#define AXI_LCD_REG_HTIMING3		0x408
#define AXI_LCD_REG_VTIMING1		0x440
#define AXI_LCD_REG_VTIMING2		0x444
#define AXI_LCD_REG_VTIMING3		0x448

#define AXI_LCD_RESET_ENABLE		BIT(0)

#define AXI_LCD_CTRL_FULL_RANGE	BIT(1)
#define AXI_LCD_CTRL_CSC_BYPASS	BIT(0)
#define AXI_LCD_CTRL_BGR (AXI_LCD_CTRL_CSC_BYPASS)
#define AXI_LCD_CTRL_RGB (AXI_LCD_CTRL_CSC_BYPASS | BIT(2))

#define AXI_LCD_SOURCE_SEL_COLORPATTERN	0x3
#define AXI_LCD_SOURCE_SEL_TESTPATTERN		0x2
#define AXI_LCD_SOURCE_SEL_NORMAL		0x1
#define AXI_LCD_SOURCE_SEL_NONE		0x0

static const struct debugfs_reg32 axi_lcd_encoder_debugfs_regs[] = {
	{ "Reset", AXI_LCD_REG_RESET },
	{ "Control", AXI_LCD_REG_CTRL },
	{ "Source select", AXI_LCD_REG_SOURCE_SEL },
	{ "Colorpattern", AXI_LCD_REG_COLORPATTERN },
	{ "Status", AXI_LCD_REG_STATUS },
	{ "VDMA status", AXI_LCD_REG_VDMA_STATUS },
	{ "TPM status", AXI_LCD_REG_TPM_STATUS },
	{ "HTiming1", AXI_LCD_REG_HTIMING1 },
	{ "HTiming2", AXI_LCD_REG_HTIMING2 },
	{ "HTiming3", AXI_LCD_REG_HTIMING3 },
	{ "VTiming1", AXI_LCD_REG_VTIMING1 },
	{ "VTiming2", AXI_LCD_REG_VTIMING2 },
	{ "VTiming3", AXI_LCD_REG_VTIMING3 },
};
...
static inline struct axi_lcd_encoder *to_axi_lcd_encoder(struct drm_encoder *enc)
{
	return container_of(enc, struct axi_lcd_encoder, encoder.base);
}

static inline struct drm_encoder *connector_to_encoder(struct drm_connector *connector)
{
	struct axi_lcd_encoder *enc = container_of(connector, struct axi_lcd_encoder, connector);
	return &enc->encoder.base;
}

static int axi_lcd_connector_init(struct drm_device *dev,
...
static int axi_lcd_debugfs_cp_get(void *data, u64 *val)
{
	struct axi_lcd_private *private = data;
	*val = readl(private->base + AXI_LCD_REG_COLORPATTERN);
	return 0;
}

static int axi_lcd_debugfs_cp_set(void *data, u64 val)
{
	struct axi_lcd_private *private = data;

	writel(val, private->base + AXI_LCD_REG_COLORPATTERN);

	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(axi_lcd_cp_fops, axi_lcd_debugfs_cp_get,
	axi_lcd_debugfs_cp_set, "0x%06llx\n");


static const char * const axi_lcd_mode_text[] = {
	[AXI_LCD_SOURCE_SEL_NONE] = "none",
	[AXI_LCD_SOURCE_SEL_NORMAL] = "normal",
	[AXI_LCD_SOURCE_SEL_TESTPATTERN] = "testpattern",
	[AXI_LCD_SOURCE_SEL_COLORPATTERN] = "colorpattern",
};

static ssize_t axi_lcd_read_mode(struct file *file, char __user *userbuf,
	size_t count, loff_t *ppos)
{
	struct axi_lcd_private *private = file->private_data;
	uint32_t src;
	const char *fmt;
	size_t len = 0;
	char buf[50];
	int i;
	src = readl(private->base + AXI_LCD_REG_SOURCE_SEL);

	for (i = 0; i < ARRAY_SIZE(axi_lcd_mode_text); i++) {
		if (src == i)
			fmt = "[%s] ";
		else
			fmt = "%s ";
...
static ssize_t axi_lcd_set_mode(struct file *file, const char __user *userbuf,
	size_t count, loff_t *ppos)
{
	struct axi_lcd_private *private = file->private_data;
	char buf[20];
	...
	for (i = 0; i < ARRAY_SIZE(axi_lcd_mode_text); i++) {
		if (sysfs_streq(axi_lcd_mode_text[i], buf))
			break;
	}

	if (i == ARRAY_SIZE(axi_lcd_mode_text))
		return -EINVAL;

	writel(i, private->base + AXI_LCD_REG_SOURCE_SEL);

	return count;
}

static const struct file_operations axi_lcd_mode_fops = {
	.open = simple_open,
	.read = axi_lcd_read_mode,
	.write = axi_lcd_set_mode,
};

static void axi_lcd_debugfs_init(struct axi_lcd_encoder *encoder)
{
	struct axi_lcd_private *priv = encoder->encoder.base.dev->dev_private;

	encoder->regset.base = priv->base;
	encoder->regset.regs = axi_lcd_encoder_debugfs_regs;
	encoder->regset.nregs = ARRAY_SIZE(axi_lcd_encoder_debugfs_regs);

	debugfs_create_regset32(dev_name(encoder->encoder.base.dev->dev), S_IRUGO, NULL, &encoder->regset);
	debugfs_create_file("color_pattern", 0600, NULL, priv, &axi_lcd_cp_fops);

	debugfs_create_file("mode", 0600, NULL, priv, &axi_lcd_mode_fops);
}

#else

static inline void axi_lcd_debugfs_init(struct axi_lcd_encoder *enc)
{
}
...
static void axi_lcd_encoder_dpms(struct drm_encoder *encoder, int mode)
{
        printk(KERN_ERR "Entered DPMS - turning %s\n", mode == DRM_MODE_DPMS_ON ? "on" : "off");
	struct axi_lcd_encoder *axi_lcd_encoder = to_axi_lcd_encoder(encoder);
	struct drm_connector *connector = &axi_lcd_encoder->connector;
...
void set_timings(struct drm_display_mode *mode, struct axi_lcd_private *private) {
	...
	writel(val,  private->base + AXI_LCD_REG_HTIMING1);
	val = mode->hsync_end - mode->hsync_start;
	writel(val,  private->base + AXI_LCD_REG_HTIMING2);
	val = (h_de_max << 16) | h_de_min;
	writel(val,  private->base + AXI_LCD_REG_HTIMING3);

	val = (mode->vdisplay << 16) | mode->vtotal;
	writel(val,  private->base + AXI_LCD_REG_VTIMING1);
	val = mode->vsync_end - mode->vsync_start;
	writel(val,  private->base + AXI_LCD_REG_VTIMING2);
	val = (v_de_max << 16) | v_de_min;
	writel(val,  private->base + AXI_LCD_REG_VTIMING3);
...
{
	axi_lcd_encoder_dpms(encoder, DRM_MODE_DPMS_ON);
}

static void axi_lcd_encoder_prepare(struct drm_encoder *encoder)
{
	axi_lcd_encoder_dpms(encoder, DRM_MODE_DPMS_OFF);
}

static struct drm_crtc *axi_lcd_encoder_get_crtc(struct drm_encoder *encoder)
{
	return encoder->crtc;
}

static struct drm_encoder_helper_funcs axi_lcd_encoder_helper_funcs = {
	.dpms		= axi_lcd_encoder_dpms,
	.mode_fixup	= axi_lcd_encoder_mode_fixup,
	.mode_set	= axi_lcd_encoder_mode_set,
	.prepare	= axi_lcd_encoder_prepare,
	.commit		= axi_lcd_encoder_commit,
	.get_crtc	= axi_lcd_encoder_get_crtc,
};

static void axi_lcd_encoder_destroy(struct drm_encoder *encoder)
{
	struct drm_encoder_slave_funcs *sfuncs = get_slave_funcs(encoder);
	struct axi_lcd_encoder *axi_lcd_encoder =
		to_axi_lcd_encoder(encoder);
	...
	kfree(axi_lcd_encoder);
}

static struct drm_encoder_funcs axi_lcd_encoder_funcs = {
...
struct drm_encoder *axi_lcd_encoder_create(struct drm_device *dev)
{
	...
	encoder = &axi_lcd_encoder->encoder.base;
	encoder->possible_crtcs = 1;
	drm_encoder_init(dev, encoder, &axi_lcd_encoder_funcs,
			DRM_MODE_ENCODER_TMDS);
	drm_encoder_helper_add(encoder, &axi_lcd_encoder_helper_funcs);
...
static int axi_lcd_connector_get_modes(struct drm_connector *connector)
{
...
static int axi_lcd_connector_mode_valid(struct drm_connector *connector,
...
static struct drm_encoder *axi_lcd_best_encoder(struct drm_connector *connector)
{
	return connector_to_encoder(connector);
}

static struct drm_connector_helper_funcs axi_lcd_connector_helper_funcs = {
	.get_modes	= axi_lcd_connector_get_modes,
	.mode_valid	= axi_lcd_connector_mode_valid,
	.best_encoder	= axi_lcd_best_encoder,
};

static enum drm_connector_status axi_lcd_connector_detect(
	struct drm_connector *connector, bool force)
{
        return connector_status_connected;
}

static void axi_lcd_connector_destroy(struct drm_connector *connector)
{
	drm_sysfs_connector_remove(connector);
	drm_connector_cleanup(connector);
}

static struct drm_connector_funcs axi_lcd_connector_funcs = {
	.dpms		= drm_helper_connector_dpms,
	.fill_modes	= drm_helper_probe_single_connector_modes,
	.detect		= axi_lcd_connector_detect,
	.destroy	= axi_lcd_connector_destroy,
};
	...
	struct drm_connector *connector, struct drm_encoder *encoder)
{
	...
	drm_connector_init(dev, connector, &axi_lcd_connector_funcs, type);
	drm_connector_helper_add(connector, &axi_lcd_connector_helper_funcs);
